  ...
  def menu_builder_for(user)
    if "Student" == user[:type]:
      [ {:label => "Startseite", :submenu_items => [link_to("Startseite", admin_path)]},
        {:label => "Benutzerdaten", :submenu_items => [
          link_to("Eigenes Profil", user_path(user)),
          (link_to("Benutzer verwalten", users_path) if user.has_right?("users", "update_all"))]},
...
#        {:label => "Jobbörse", :submenu_items => [
#          link_to("Stellenangebote", jobs_path)]},
