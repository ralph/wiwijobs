...
class RolesControllerTest < Test::Unit::TestCase
  fixtures :roles, :users, :rights, :rights_roles

  def setup
    @controller = RolesController.new
    @request    = ActionController::TestRequest.new
    @response   = ActionController::TestResponse.new
    %w(index show new edit create update destroy).each {|action| Right.create(:name => "Roles", :controller => "roles", :action => action)}
    new_rights = Right.find_all_by_controller("roles")
  ...
  def test_should_create_role
    ...
    post :create, :role => { "name" => "systemname", "title" => "Some more descriptive title"}
